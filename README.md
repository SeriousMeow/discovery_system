...
В поле "Discovery server URL" необходимо указать URL сервера, которым будет пользоваться клиент. В примере на изображении это http://172.29.0.11:8080 (нужен адрес во внутренней сети docker). Также сожно указать свой публичный ключ в discovery системе, чтобы продолжить работу тем же пользователем, что и раньше. Если оставить поле пустым, то будет сгенерирован новый ключ.
...
В секции **Online** можно узнать статус подключения к серверу. В режиме "Online" клиент начинает получать данные с сервера о входящих соединениях. После перехода в статус "Online" можно получить свой ID для соединения с другим клиентом. Также можно получить свой публичный ключ в discovery системе, который был описан ранее.
